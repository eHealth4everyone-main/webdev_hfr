...
<h4> <p class="text-aqua">Unique ID: {{$imagings->sig_unique_id}}</p></h4>
@endsection
...
<form class="form-horizontal" action="/imaging/{{$imagings->id}}" method="POST">
    @csrf
    @method("PUT")
        ...
        <div class="panel panel-primary">
            <div class="panel-heading" role="tab" id="headingOne">
                <h4 class="panel-title">
                    <a role="button" data-toggle="collapse" data-parent="#accordion" href="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
                        Edit Radiological Facility Details
                    </a>
                </h4>
            </div>
            <div id="collapseOne" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="headingOne">
                <div class="panel-body">
                    <div class="box-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-4">
                                <input type="text" class="form-control"  id="cac_reg" name="cac_reg" value="{{$imagings->cac_reg}}" placeholder="Corporate Affairs Registration Number">
                            </div>
                            ...
                            <div class="col-sm-4">
                                <div class="input-group date" >
                                    ...
                                    <input type="text" class="form-control pull-right" id="datepicker"  value="{{$imagings->comm_date}}" name="comm_date">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-4">
                                <input type="text" class="form-control"  id="reg_fac_name"  name="reg_fac_name" value="{{$imagings->reg_fac_name}}" placeholder="Registered Facility Name">
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-4">
                                <input type="text" class="form-control"  id="latitude" name="latitude"  value="{{$imagings->latitude}}">
                            </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control"  id="hs_ownership_details" name="hs_ownership_details" value="{{$imagings->imaging->im_detail}}">
                        </div>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-4">
                        <input type="text" class="form-control"  id="im_regg_num" name="im_regg_num" value="{{$imagings->imaging->im_regg_num}}" placeholder="Radiographers Council Physical Premise Reg No">                        
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-4">
                        <input type="text" class="form-control input-sm"  id="im_radiologist" name="im_radiologist"  value="{{$imagings->imaging->im_radiologist}}">
                    </div>
                    <label for="hs_no_lab_sc" class="col-sm-2 control-label">Number of Radiographers:</label>
                    <div class="col-sm-4">
                        <input type="text" class="form-control input-sm"  id="im_radiographer" name="im_radiographer" value="{{$imagings->imaging->im_radiographer}}">
                    </div>
                </div>
                <div class="form-group">
                    <label for="hs_no_single_qualified_nurses" class="col-sm-2 control-label">Radiography Technicians:</label>
                    <div class="col-sm-4">
                        <input type="text" class="form-control input-sm"  id="im_radiography_tech" name="im_radiography_tech"  value="{{$imagings->imaging->im_radiography_tech}}">
                    </div>
                </div>
...
<script>
    $(document).ready(function(){
        var state1 = "{{$imagings->state}}";
        var lga1 = "{{$imagings->lga}}";
        ...
        if(lga1 != '')
        {
            ...
            $.ajax({
                ...
                {
                    $('#ward').html(result);
                    $("#ward").val("{{$imagings->ward}}");
                }         
            })
        }
        //fill imaging services
        var servic = "{{$imagings->imaging->im_service}}";
        var _token = $('input[name="_token"]').val();
        $.ajax({
            ...
            success:function(result)
            {
                ...
                $('#fac_services').val(servic.split(','));
            }         
        })
        
        $("#hr_operation").val("{{$imagings->hr_operation}}");
        
        var values = "{{$imagings->operational_days}}";
        $('#opsdays').val(values.split(','));
        
        
        $("#hs_ownership").val("{{$imagings->imaging->im_owner}}");
        $("#hs_op_status").val("{{$imagings->imaging->im_op_status}}");
        $("#hs_reg_status").val("{{$imagings->imaging->im_reg_status}}");
        $("#hs_lic_status").val("{{$imagings->imaging->im_lic_status}}");
        $("#standalone").val("{{$imagings->imaging->im_state}}");
        
      
        
        //fill ownership type
        if("{{$imagings->imaging->im_owner_type}}"!="")
        {    
            $('#hs_ownership_type option').val('{{$imagings->imaging->im_owner_type}}').html('{{$imagings->imaging->im_owner_type}}');                
        }
